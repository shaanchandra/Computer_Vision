...
for ii = 1:length(lambdas)
    for jj  = 1:length(sigmas)
        for ll = 1:length(orientations)
            ...
            gaborFilterBank(filterNo).filterPairs = createGabor_mahsa( sigma, theta, lambda, psi, gamma );
...
if smoothingFlag
    for jj = 1:length(featureMags)
        features(:,:,jj) =  imgaussfilt(featureMags{jj}, 4*sigma);
    end
else
...
% features. 
tic
pixLabels = kmeans(features,2,'Replicates',5); % \\TODO: Return cluster labels per pixel
ctime = toc;
...
title('Masked Object and Background')