...
for jj = 1 : length(gaborFilterBank)
    ...
    real_out = imfilter(img_gray,filter(:,:,1));
    imag_out =  imfilter(img_gray,filter(:,:,2));
...
% \\ Q: What kind of features do you think gabor filters might correspond
% to? answered in the report.
...
if smoothingFlag
    ...
    for i = 1:length(featureMags)
        featureMags{i} = imgaussfilt(featureMags{i}, 10*gaborFilterBank(i).sigma);
