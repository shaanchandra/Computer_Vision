...
%kernel size for the first order gaussian in the final smoothing
kernel_size = 3;
...
for ii = 1:length(lambdas)
    for jj  = 1:length(sigmas)
        for ll = 1:length(orientations)
            ...
            gaborFilterBank(filterNo).filterPairs = createGabor_rad( sigma, theta, lambda, psi, gamma );
...
for jj = 1 : length(gaborFilterBank)
    real_out =  imfilter(img_gray, gaborFilterBank(jj).filterPairs(:,:,1),'circular','conv');
    imag_out =  imfilter(img_gray, gaborFilterBank(jj).filterPairs(:,:,2),'circular','conv');
...
for jj = 1:length(featureMaps)
    ...
    featureMags{jj} = (double(real_part.^2 + imag_part.^2)).^(0.5);
...
if smoothingFlag
    ...
    for i = 1:length(featureMags)
        %G = gauss1D(0.5*gaborFilterBank(i).sigma,kernel_size);
        features(:,:,i) = imgaussfilt(featureMags{i},10*gaborFilterBank(i).sigma);
%         features(:,:,i) = featureMags{i};
    end
else
...
%     summed_all_features = sqrt(sum(features.^2,2));
%     pixLabels = kmeans(summed_all_features(:),k); % \\TODO: Return cluster labels per pixel
%     ctime = toc;
...
% figure(6)
% %i = [7,10,19,22,31,34]
% for i = [8,11,20,22,32,35]
%     current_lambda = gaborFilterBank(i).lambda;
%     current_sigma = gaborFilterBank(i).sigma;
%     current_tetha = gaborFilterBank(i).theta;
%     
%     X = 1:numCols;
%     Y = 1:numRows;
%     [X,Y] = meshgrid(X,Y);
...
%     imshowpair(Aseg1,Aseg2,'montage')
%     title(['lambda = ',num2str(current_lambda),'    sigma = ',num2str(current_sigma), '    theta = ',num2str(current_tetha)]);
